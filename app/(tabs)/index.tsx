...
export default function FocusFlowTimer({ todayMinutes = 0 }: TimerScreenProps) {
  ...
  const { isRunning, startSession, finishSession, cancelCurrentSession } =
    useTimer();
  ...
  const handleSliderChange = (value: number) => {
    ...
    setMinutes(rounded);
    setSecondsRemaining(rounded * 60);
  ...
  const progress = isRunning ? secondsRemaining / totalSeconds : 1;
